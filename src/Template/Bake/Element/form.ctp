...
<h1 id="wb-cont" property="name">
    ...
    <span class="pull-right">
        ...
        <?= $this->Html->link(__('View'), ['action' => 'view', <%= $pk %>], ['class' => "btn btn-default"]) ?>
    </span>
</h1>
<% endif; %>
...
<% if (strpos($action, 'add') === false): %>
            <li><?= $this->Form->postLink(
                    ...
                    ['confirm' => __('Are you sure you want to delete # {0}?', $<%= $singularVar %>-><%= $primaryKey[0] %>), 'class' => "list-group-item list-group-item-danger"]
                )
            ?></li>
<% endif; %>
